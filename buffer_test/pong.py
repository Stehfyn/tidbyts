...
def game_loop():
    ...
    while True:
        ...
        temp_surface = pygame.transform.smoothscale(g_screen, (64,32))
        temp_surface = pygame.transform.flip(temp_surface, True, False)
        temp_surface = pygame.transform.rotate(temp_surface, 90.0)
        temp_surface.set_alpha(125)

        temp_surface = pygame.surfarray.array3d(temp_surface)
        _, surface_encode = cv2.imencode('.png', temp_surface)
        g_screen_buffer = base64.b64encode(surface_encode)
...
class pygameSurfaceHTTP(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        print(g_screen_buffer)
        screen_buffer_str=str(g_screen_buffer).replace("'","")
        screen_buffer_str = screen_buffer_str[1:]
        self.wfile.write(bytes('<html><body><h1 id="A">'+ screen_buffer_str +'</h1></body></html>', 'utf-8'))
...
def main():
    ...
    print(LOCALHOST)
    g_server = HTTPServer((LOCALHOST, PORT), pygameSurfaceHTTP)
    surface_thread = threading.Thread(target=g_server.serve_forever, args=())
    surface_thread.daemon = True
    surface_thread.start()
    print("Surface Server now running")
